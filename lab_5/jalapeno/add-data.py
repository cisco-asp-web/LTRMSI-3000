...
if db.has_collection('fabric_node'):
    fabric = db.collection('fabric_node')
...
def insert_host_data(db, host_file_path):
    """
    Insert host data from hosts.json into the jalapeno database
    Args:
        db: ArangoDB database connection
        host_file_path: Path to the hosts.json file
    """
    try:
        # Read the host data from JSON file
        with open(host_file_path, 'r') as f:
            host_data = json.load(f)
        
        # Ensure hosts collection exists
        if not db.has_collection('hosts'):
            db.create_collection('hosts')
        
        host_collection = db.collection('hosts')
        
        # AQL query to insert/update host data
        aql = """
        FOR host in @hosts
        ...
        print(f"Error inserting host data: {str(e)}")
...
def insert_host_edges_v6(db, edge_file_path):
    """
    Insert host IPv6 edge data from host-edge-v6.json into the ipv6_graph collection
    Args:
        db: ArangoDB database connection
        edge_file_path: Path to the host-edge-v6.json file
    """
    try:
        # Read the edge data from JSON file
        with open(edge_file_path, 'r') as f:
        ...
        # Execute AQL query
        db.aql.execute(aql, bind_vars={'edges': edge_data})
        print(f"Successfully inserted/updated {len(edge_data)} host IPv6 edge records")
        
    except Exception as e:
        print(f"Error inserting host IPv6 edge data: {str(e)}")
...
if __name__ == "__main__":
    ...
    if not args.data or 'all' in args.data:
        ...
        if 'gpu' in args.data:
            print("\nInserting GPU data...")
            insert_gpu_data(db, "gpus.json")
        if 'host' in args.data:
            print("\nInserting host data...")
            insert_host_data(db, "hosts.json")
        if 'gpu-edges' in args.data:
            print("\nInserting GPU IPv6 edge data...")
            insert_gpu_edges_v6(db, "gpu-edge-v6.json")
        if 'host-edges' in args.data:
            print("\nInserting host IPv6 edge data...")
            insert_host_edges_v6(db, "host-edge-v6.json")
